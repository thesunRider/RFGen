 ...
 */
	...
	/** Hardware brief **/    
	//PORTB						//DDRx		PORTx		PINx pin on nano - 12
	#define	_TCLK    PB13		// 1          1          0
	
	//PORTD
	#define	_TDAT	 PB12		// 1          0 pin on nano - 6

	#define	SetTCLK()	(digitalWrite(_TCLK,HIGH))
	#define	ClrTCLK()	(digitalWrite(_TCLK,LOW))
	
	#define	InputTDAT() (pinMode(_TDAT,INPUT))
	#define	OutputTDAT() (pinMode(_TDAT,OUTPUT))
	
	#define	SetTDAT()	(digitalWrite(_TDAT,HIGH))
	#define	ClrTDAT()	(digitalWrite(_TDAT,LOW))
	
	#define TDAT_H()	(digitalRead(_TDAT))
