...
#define BIAS_PIN1 PA2
#define BIAS_PIN2 PA3
#define BIAS_PIN3 PB10
#define BUZZ_PIN PB11
#define PWM_PIN2 PB8
#define PWM_PIN1 PA8

#define ROT_SELECT_CLK PB1
#define ROT_SELECT_DT PB0
#define ROT_SELECT_SW PA7

#define ROT_MODE_CLK PA5
#define ROT_MODE_DT PA6
#define ROT_MODE_SW PA4


// Pin mapping for the display:
const byte LCD_RS = PB14;
const byte LCD_E = PB15;
const byte LCD_D4 = PB5;
const byte LCD_D5 = PB4;
const byte LCD_D6 = PB3;
const byte LCD_D7 = PA15;
...
TIM_TypeDef *Instanc_PWM1 = (TIM_TypeDef *)pinmap_peripheral(pinNameToUse_PWM1, PinMap_PWM);
uint32_t PWM_channel1 = STM_PIN_CHANNEL(pinmap_function(pinNameToUse_PWM1, PinMap_PWM));

PinName pinNameToUse_PWM2 = digitalPinToPinName(PWM_PIN2);
TIM_TypeDef *Instanc_PWM2 = (TIM_TypeDef *)pinmap_peripheral(pinNameToUse_PWM2, PinMap_PWM);
uint32_t PWM_channel2 = STM_PIN_CHANNEL(pinmap_function(pinNameToUse_PWM2, PinMap_PWM));


HardwareTimer *PWM1;
HardwareTimer *PWM2;

void setPWMfrequency(int chn, uint32_t freq, int duty) {
  if (chn == 0) {
    if (PWM1)
      delete PWM1;

    PWM1 = new HardwareTimer(Instanc_PWM1);
    PWM1->setPWM(PWM_channel1, PWM_PIN1, freq, duty);
  } else {
    if (PWM2)
      delete PWM2;

    PWM2 = new HardwareTimer(Instanc_PWM2);
    PWM2->setPWM(PWM_channel2, PWM_PIN2, freq, duty);
  }
}
...
void page_chnA() {
  ...
  while (!(was_clicked(SELECT_SW, false) && option_selected == BACK_OPTION)) {
    ...
    if (was_clicked(SELECT_SW, false) && option_selected == CHN_BIAS) {
      ...
      bias_chnA = !bias_chnA;
      bias_chnA ? lcd.print("B") : lcd.write(byte(BIAS_MENU));
      bias_chnA ? digitalWrite(BIAS_PIN1, HIGH) : digitalWrite(BIAS_PIN1, LOW);
    }
...
void page_chnC() {
  ...
  if (enable_c) {
    ...
    setPWMfrequency(0,freq_apply_c, duty_c);
  } else {
...
void page_chnD() {
  ...
  if (enable_d) {
    ...
    setPWMfrequency(1,freq_apply_d, duty_d);
  } else {
...
void setup() {
  ...
  pinMode(BIAS_PIN1, OUTPUT);
  pinMode(BIAS_PIN2, OUTPUT);
  pinMode(BIAS_PIN3, OUTPUT);
  pinMode(BUZZ_PIN, OUTPUT);

  digitalWrite(BIAS_PIN1, LOW);
  digitalWrite(BIAS_PIN2, LOW);
  digitalWrite(BIAS_PIN3, LOW);
  digitalWrite(BUZZ_PIN, HIGH);
